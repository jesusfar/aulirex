...
export function DashboardPage() {
  ...
  return (
    <section className="space-y-6">
      <div className="exam-countdown-panel overflow-hidden rounded-lg border border-green-200/20 px-5 py-5 shadow-[0_0_42px_rgba(16, 185, 129,0.18)] sm:px-8 sm:py-6">
        <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-green-100/80">
            ...
            {countdown.detail && (
              <p className="mt-2 text-sm font-semibold text-green-100/75">
                {countdown.detail}
              </p>
            )}
          </div>
          ...
          <label className="shrink-0">
            ...
            <input
              ...
              className="exam-countdown-input h-12 w-full min-w-44 rounded-md border border-green-100/30 bg-slate-950/72 px-4 text-sm font-black text-green-50 outline-none transition focus:border-green-100 focus:ring-4 focus:ring-green-300/18 sm:w-48"
            />
          </label>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="rounded-lg border border-white/10 bg-slate-900/72 p-4 sm:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="aulirex-gradient-number aulirex-gradient-number--emerald text-3xl font-black">
                {lvl.level}
              </p>
            </div>
          </div>
        ...
        <Link
          to="/ingreso-gendarmeria/repaso"
          className="rounded-lg border border-white/10 bg-slate-900/72 p-4 text-center transition hover:border-emerald-400/40"
        >
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">
            Para repasar
          </p>
          <p className="mt-1 text-3xl font-black text-green-300">{dueCount}</p>
          <p className="text-xs text-slate-400">
      ...
      <div className="grid gap-4 lg:grid-cols-2 lg:items-stretch">
        <div className="rounded-lg border border-white/10 bg-slate-900/72 p-5">
          ...
          <p className="mt-3 text-xs leading-5 text-slate-500">
            Estimación sobre lo que practicaste, con curva de olvido. El ingreso
            se define por orden de mérito en los exámenes de admisión.
          </p>
        </div>
          ...
          <ul className="mt-4 space-y-2.5">
            {todayPlan.map((step) => (
              <li key={step.title}>
                <Link
                  to={step.to}
                  className="flex items-center gap-3 rounded-md border border-white/10 bg-slate-950/40 px-3 py-2.5 transition hover:border-emerald-400/40"
                >
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
        <div className="rounded-lg border border-white/10 bg-slate-900/72 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
          <div className="mb-7 max-w-2xl">
            <div className="mb-4 inline-flex items-center rounded-md border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-emerald-200">
              Banco activo
            </div>
        ...
        <aside className="rounded-lg border border-white/10 bg-black/28 p-5 shadow-xl shadow-black/20 backdrop-blur-xl">
          <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div>
              ...
              <p className="aulirex-gradient-number aulirex-gradient-number--emerald mt-1 text-3xl font-black">{bankProgress}%</p>
            </div>
            <div className="grid size-16 place-items-center rounded-full border border-emerald-300/30 bg-emerald-400/10 text-sm font-black text-emerald-200 shadow-[0_0_28px_rgba(16, 185, 129,0.18)]">
              <span className="aulirex-gradient-number aulirex-gradient-number--emerald">{stats.solved}</span>
            </div>
          </div>

          <div className="mt-5 space-y-4">
            <ProgressRow label="Dominados" value={bankProgress} tone="bg-emerald-400" />
  ...
  if (!diagnostic) {
    return (
      <div className="flex flex-col rounded-lg border border-white/10 bg-slate-900/72 p-5">
        ...
        <Link
          to="/ingreso-gendarmeria/diagnostico"
...
function masteryTone(v: number): string {
  ...
  if (v >= 0.6) return 'bg-emerald-400';
...
function Stat({
  ...
  const accentClass = {
    sky: 'bg-emerald-400/10 border-emerald-300/20 shadow-[0_0_26px_rgba(16, 185, 129,0.12)]',
    emerald: 'bg-emerald-400/10 border-emerald-300/20 shadow-[0_0_26px_rgba(16,185,129,0.12)]',
    amber: 'bg-amber-400/10 border-amber-300/20 shadow-[0_0_26px_rgba(251,191,36,0.11)]',
  }[accent];
  const numberClass = {
    sky: 'aulirex-gradient-number--emerald',
