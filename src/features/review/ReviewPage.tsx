...
export function ReviewPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <section className="rounded-lg border border-sky-300/15 bg-slate-900/68 p-5 shadow-2xl shadow-black/25 backdrop-blur-xl sm:p-6">
...
function HeaderStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="min-w-24 px-4 py-3">
      <div className="aulirex-gradient-number aulirex-gradient-number--sky text-xl font-black">{value}</div>
...
function MisconceptionCard({ group }: { group: ReviewMisconceptionGroup }) {
  return (
    <article className="rounded-lg border border-white/10 bg-slate-950/58 p-4">
      ...
      {group.remedy && (
        <p className="mt-3 rounded-md border border-cyan-300/15 bg-cyan-400/5 px-3 py-2 text-sm leading-6 text-cyan-100">
